...
typedef void (*register_module_fn)(const char*, uint32_t, uint32_t);
typedef void (*register_pc_table_fn)(const char*, const hfuzz_pc_entry_t*, size_t, uint32_t);
...
static void resolve_metrics_functions(void) {
    ...
    if (fn_session_init) {
        ...
        if (bridge_lib) {
            fprintf(stderr, "[hfuzz_metrics] ⚠️  Metrics bridge library loaded but symbols not found (version mismatch?)\n");
            dlclose(bridge_lib);
        } else {
            fprintf(stderr, "[hfuzz_metrics] ℹ️  Metrics bridge library not loaded, metrics disabled (this is OK)\n");
        }
    }
}
...
void hfuzz_metrics_register_pc_table(const char* module_name,
                                      ...
                                      size_t pc_count,
                                      uint32_t guard_start) {
    if (fn_register_pc_table) fn_register_pc_table(module_name, pcs, pc_count, guard_start);
}
